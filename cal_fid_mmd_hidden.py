"""
change output from D to include time_comb:
    discriminator = Model(
        inputs=[i_et, i_ts],
        outputs=[real_prob, time_comb])
...
Y = discriminator([g1_et, g1_ts])[1]

fid = (calculate_FID_batch(X_pos.numpy(), Y.numpy()))
mmd = (mmd.rbf_mmd2(X_pos, Y))
